...
from common.models import db, RestaurantDineInOrderDao, RestaurantOrderDao


def test_helper_delete_order(restaurant_order_id):
    db.session.query(RestaurantOrderDao).filter(
        and_(RestaurantOrderDao.id == restaurant_order_id)).delete(synchronize_session='fetch')
    db.session.commit()
...
def test_helper_create_order(client, restaurant_bill_id='11111111-1111-1111-1111-111111111111',
    ...
    return client.post(f'/api/v1/restaurants/{restaurant_id}/bills/{restaurant_bill_id}/orders', data=data,
                       headers=session)
